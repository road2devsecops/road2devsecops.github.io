...
Setelah belajar cara melakukan ekstraksi kolom, kita juga dapat memodifikasi output yang kita inginkan, sebagai contoh :  
`Command :`
``` sh
...
Sekarang untuk menandakan konfigurasi ini digereate oleh awk kita dapat memanfaatkan statment `BEGIN`.  
`Command :`
``` sh
...
for i in $(awk -F ',' '/192/{print $1}' $FILE);
  do 
    echo "Configuring $i";
    cat config | ssh root@$i 'cat >> /etc/hosts';
done
```
